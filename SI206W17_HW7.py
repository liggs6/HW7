...
table = "CREATE TABLE IF NOT EXISTS Tweets (tweet_id INT PRIMARY KEY, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets INT)"
...
for x in umsi_tweets:
	...
	data = (id, author, time, text, retweet)
	data_lst.append(data)

for tweet in data_lst:
	db_cur.execute(statement, tweet)
...
tweet_times = db_cur.execute("SELECT time_posted FROM Tweets")
tweet_posted_times = tweet_times.fetchall()
...
rts = db_cur.execute('SELECT * FROM Tweets WHERE retweets > 2')
more_than_2_rts = rts.fetchall()
...
rt = db_cur.execute("SELECT tweet_text FROM Tweets WHERE instr(tweet_text, 'RT')")
first = rt.fetchone()
first_rt = first[0]
...
def get_twitter_users(string):
	names_set = (re.findall(r'@([0-9A-z_]+)',string))
	return {x for x in names_set}
